...
3> PYTHONPATH: import 时寻找模块的路径
4> sys.path : 是一个路径列表，在程序运行期间 动态增加的。

动态修改 sys.path:
os.path.dirname(): 获取某个路径的父级路径,通常用于获取当前模块的父级路径
...
sys.path.append(os.path.dirname(__file__) + r"/files")
import regi
regi.reg("github")
regi.login()